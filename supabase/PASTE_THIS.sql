...
-- REPORTS: drop and recreate to fix missing columns
...
DROP POLICY IF EXISTS "Full access blocks" ON blocks;
CREATE POLICY "Full access blocks" ON blocks FOR ALL USING (true);
DROP POLICY IF EXISTS "Full access boosts" ON boosts;
CREATE POLICY "Full access boosts" ON boosts FOR ALL USING (true);
...
DROP POLICY IF EXISTS "Full access settings" ON user_settings;
CREATE POLICY "Full access settings" ON user_settings FOR ALL USING (true);
DROP POLICY IF EXISTS "Full access location_history" ON user_location_history;
CREATE POLICY "Full access location_history" ON user_location_history FOR ALL USING (true);
DROP POLICY IF EXISTS "Full access passport" ON passport_history;
CREATE POLICY "Full access passport" ON passport_history FOR ALL USING (true);
...
BEGIN
  ...
  WHERE p.user_id != p_user_id
    ...
    AND p.user_id NOT IN (SELECT s.swiper_id FROM swipes s WHERE s.swiper_id = p_user_id)
...
DECLARE
  ...
  v_reports integer;
BEGIN
  SELECT COUNT(*) INTO v_dates FROM date_confirmations
    WHERE (requester_id = p_user_id OR confirmer_id = p_user_id) AND both_confirmed = true;
  ...
  v_score := v_score + CASE
    WHEN v_avg_rating >= 4.5 THEN 15
    WHEN v_avg_rating >= 4 THEN 10
    WHEN v_avg_rating >= 3 THEN 5
    ELSE 0
  END;
