    ...
    const handlePay = async (currency: "USDC" | "WLD") => {
      ...
      try {
        ...
        const payload = {
          reference,
          to: RECIPIENT,
          tokens: [{ symbol: currency === "USDC" ? Tokens.USDC : Tokens.WLD, token_amount: tokenToDecimals(parseFloat(amount), currency === "USDC" ? Tokens.USDC : Tokens.WLD).toString() }],
          description: "H Love Premium - 1 Month",
        };
        const result = await MiniKit.commandsAsync.pay(payload);
        if (!result?.finalPayload || (result.finalPayload as any).status === "error") { setError("Payment cancelled"); return; }
        const txId = (result.finalPayload as any).transaction_id || reference;
        const res = await fetch("/api/confirm-payment", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ user_id: userId, payment_type: "subscription", currency, amount: parseFloat(amount), tx_id: txId, reference }),
        });
        const data = await res.json();
        if (data.success) onPurchased();
        else setError(data.error || "Payment confirmation failed");
      } catch (err) { setError(err instanceof Error ? err.message : "Payment failed"); } finally { setPaying(false); }
    };
