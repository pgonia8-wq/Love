import { useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { Crown, Heart, Sparkles, Zap, Shield, X, Plane, Navigation, EyeOff, Ban } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useI18n } from "@/lib/i18n";
import { MiniKit, tokenToDecimals, Tokens } from "@worldcoin/minikit-js";

interface PremiumModalProps {
  open: boolean;
  onClose: () => void;
  userId: string;
  onPurchased: () => void;
}

const RECIPIENT = "0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045";

export default function PremiumModal({ open, onClose, userId, onPurchased }: PremiumModalProps) {
  const { t } = useI18n();
  const [paying, setPaying] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const handlePay = async (currency: "USDC" | "WLD") => {
    console.log("[PremiumModal] handlePay started. currency:", currency, "userId:", userId);

    if (!MiniKit.isInstalled()) {
      console.error("[PremiumModal] MiniKit not installed");
      setError("Abre la app dentro de World App");
      return;
    }

    setPaying(true);
    setError(null);

    try {
      const amount = currency === "USDC" ? "9.99" : "3.5";
      const token = currency === "USDC" ? Tokens.USDC : Tokens.WLD;
      const reference = "hlove_premium_" + Date.now() + "_" + Math.random().toString(36).slice(2, 8);

      console.log("[PremiumModal] Creating payment payload. amount:", amount, "token:", token, "reference:", reference);

      const payload = {
        reference,
        to: RECIPIENT,
        tokens: [{
          symbol: token,
          token_amount: tokenToDecimals(parseFloat(amount), token).toString(),
        }],
        description: "H Love Premium - 1 Month",
      };

      console.log("[PremiumModal] Calling MiniKit.commandsAsync.pay...");
      const result = await MiniKit.commandsAsync.pay(payload);
      console.log("[PremiumModal] pay result status:", result?.finalPayload?.status);

      if (!result?.finalPayload || (result.finalPayload as any).status === "error") {
        console.error("[PremiumModal] Payment cancelled or failed:", JSON.stringify(result?.finalPayload));
        setError("Pago cancelado");
        setPaying(false);
        return;
      }

      const txId = (result.finalPayload as any).transaction_id || reference;
      console.log("[PremiumModal] Payment completed. tx_id:", txId);
      console.log("[PremiumModal] Sending to backend /api/confirm-payment...");

      const res = await fetch("/api/confirm-payment", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          user_id: userId,
          payment_type: "subscription",
          currency,
          amount: parseFloat(amount),
          tx_id: txId,
          reference,
        }),
      });

      const data = await res.json();
      console.log("[PremiumModal] /api/confirm-payment response:", JSON.stringify(data));

      if (data.success) {
        console.log("[PremiumModal] Premium activated!");
        onPurchased();
      } else {
        console.error("[PremiumModal] Backend rejected:", data.error);
        setError(data.error || "Error confirmando pago");
      }
    } catch (err) {
      console.error("[PremiumModal] Exception:", err);
      setError(err instanceof Error ? err.message : "Error en el pago");
    } finally {
      setPaying(false);
    }
  };

  const features = [
    { icon: Heart, text: t("premium.unlimitedLikes"), color: "text-love-pink" },
    { icon: Sparkles, text: t("premium.seeWhoLiked"), color: "text-love-gold" },
    { icon: Zap, text: t("premium.freeBoost"), color: "text-love-purple" },
    { icon: Shield, text: t("premium.priorityFeed"), color: "text-green-500" },
    { icon: Plane, text: t("premium.travelMode"), color: "text-blue-500" },
    { icon: Navigation, text: t("premium.changeCountry"), color: "text-orange-500" },
    { icon: EyeOff, text: t("premium.incognitoMode"), color: "text-gray-500" },
    { icon: Ban, text: t("premium.noAds"), color: "text-red-400" },
  ];

  return (
    <AnimatePresence>
      {open && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
          <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }}
            onClick={(e) => e.stopPropagation()} className="bg-card rounded-t-3xl sm:rounded-3xl p-6 max-w-md w-full shadow-2xl border border-border/50 max-h-[90vh] overflow-auto relative">
            <button onClick={onClose} className="absolute top-4 right-4 text-muted-foreground hover:text-foreground"><X className="w-5 h-5" /></button>

            <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-love-gold to-love-pink flex items-center justify-center mx-auto mb-5 shadow-xl">
              <Crown className="w-10 h-10 text-white" />
            </div>
            <h3 className="text-2xl font-bold text-center mb-1">{t("premium.unlock")}</h3>
            <p className="text-sm text-muted-foreground text-center mb-6">{t("premium.description")}</p>
            <div className="space-y-2 mb-6">
              {features.map((f, i) => (
                <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }} className="flex items-center gap-3 px-4 py-2.5 bg-muted/30 rounded-xl">
                  <f.icon className={"w-4 h-4 " + f.color} /><span className="text-sm font-medium">{f.text}</span>
                </motion.div>
              ))}
            </div>
            <div className="space-y-3">
              <Button className="w-full h-13 bg-gradient-to-r from-love-gold to-love-pink border-0 rounded-xl font-semibold text-base text-white shadow-lg" onClick={() => handlePay("USDC")} disabled={paying}>
                {paying ? <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: "linear" }} className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full" /> : <><Crown className="w-5 h-5 mr-2" />{t("premium.upgrade")}</>}
              </Button>
              <Button variant="outline" className="w-full h-11 rounded-xl text-sm" onClick={() => handlePay("WLD")} disabled={paying}>
                <Sparkles className="w-4 h-4 mr-2 text-love-gold" />{t("premium.orWLD")} — 3.5 WLD
              </Button>
            </div>
            <AnimatePresence>{error && <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="mt-3 text-sm text-destructive text-center">{error}</motion.p>}</AnimatePresence>
            <button onClick={onClose} className="w-full mt-3 text-xs text-muted-foreground text-center py-2">{t("premium.maybeLater")}</button>
          </motion.div>
        </motion.div>
      )}
    </AnimatePresence>
  );
}
