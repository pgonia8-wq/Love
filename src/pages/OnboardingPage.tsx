import { useState, useCallback } from "react";
  ...
  export default function OnboardingPage({ userId, username, onComplete }: OnboardingPageProps) {
    ...
    const handleSave = async () => {
      ...
      setError(null);
      try {
        console.log("[Onboarding] Saving profile for wallet:", userId.slice(0, 10));
        ...
        if (data) {
          console.log("[Onboarding] Profile saved:", data.display_name);
          onComplete(data);
        }
      } catch (err) {
      ...
      switch (step) {
        case 0:
          return (
            ...
            parseInt(form.age) >= MIN_AGE &&
            form.gender !== ""
          );
        case 1:
          return form.bio.trim().length >= 10 && form.interests.length >= 3;
        case 2:
          return form.photos.length >= 1;
        case 3:
          return form.city.trim().length >= 2 && form.looking_for !== "";
        default:
          return false;
      }
    };
    ...
    const slides = [
              ...
              <button
                ...
                className={`py-3 rounded-xl text-sm font-medium transition-all ${
                  form.gender === g.toLowerCase()
                    ...
                    : "bg-card text-foreground/70 border border-border/50 hover:border-love-pink/30"
                }`}
              >
                {g}
              </button>
      ...
      <div key="about" className="space-y-5">
        <div>
          ...
          <Textarea
            ...
            className="min-h-[120px] bg-card border-border/50 rounded-xl resize-none"
            maxLength={500}
          />
              ...
              <button
                ...
                className={`px-3.5 py-1.5 rounded-full text-sm transition-all ${
                  form.interests.includes(interest)
                    ? "gradient-love text-white shadow-sm"
                    : "bg-card text-foreground/60 border border-border/50 hover:border-love-pink/30"
                }`}
              >
                {interest}
              </button>
      ...
      <div key="photos" className="space-y-5">
        <p className="text-sm text-muted-foreground">
          Add up to {MAX_PHOTOS} photos. Your first photo will be your main profile picture.
        </p>
        <div className="grid grid-cols-3 gap-3">
          {form.photos.map((photo, i) => (
            <div key={i} className="relative aspect-[3/4] rounded-xl overflow-hidden group">
              ...
              <button
                ...
                className="absolute top-1.5 right-1.5 w-6 h-6 bg-black/60 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity text-white text-xs"
              >
                X
              </button>
              {i === 0 && (
          ...
          {form.photos.length < MAX_PHOTOS && (
            <label className="aspect-[3/4] rounded-xl border-2 border-dashed border-border/50 flex flex-col items-center justify-center cursor-pointer hover:border-love-pink/40 transition-colors">
      ...
      <div key="preferences" className="space-y-5">
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {["Men", "Women", "Everyone", "Friends"].map((pref) => (
              <button
                key={pref}
                onClick={() => updateField("looking_for", pref.toLowerCase())}
                className={`py-3 rounded-xl text-sm font-medium transition-all ${
                  form.looking_for === pref.toLowerCase()
                    ? "gradient-love text-white shadow-md"
                    : "bg-card text-foreground/70 border border-border/50 hover:border-love-pink/30"
                }`}
              >
      ...
      <div className="min-h-screen flex flex-col px-6 py-8">
        ...
        <div className="flex-1 flex flex-col">
          <motion.h2
            ...
            className="text-2xl font-bold mt-6 mb-6 gradient-love-text"
          >
            {stepTitles[step]}
          </motion.h2>
          ...
          <div className="flex gap-3 mt-8 pb-4">
            ...
            <Button
              onClick={step === totalSteps - 1 ? handleSave : () => setStep((s) => s + 1)}
              disabled={!canProceed() || saving}
              className="flex-1 h-12 gradient-love border-0 rounded-xl font-semibold"
            >
              {saving ? (
                <motion.div
                  animate={{ rotate: 360 }}
