    ...
    <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column" }}>
      ...
      {showLimitModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 60, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.85)", padding: "0 32px" }}>
          <div style={{ background: "#1c1c1e", borderRadius: 24, padding: 24, maxWidth: 320, width: "100%", textAlign: "center" }}>
            ...
            <button onClick={() => setShowLimitModal(false)} style={{ color: "#fff", fontSize: 14, background: "#ec4899", border: "none", padding: "10px 24px", borderRadius: 12, fontWeight: 600, cursor: "pointer" }}>OK</button>
          </div>
        </div>
      )}
      ...
      {showMatch && (
        <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.9)", padding: "0 32px" }}>
          ...
          <button onClick={() => setShowMatch(null)} className="gradient-love" style={{ color: "#fff", padding: "12px 32px", borderRadius: 16, fontWeight: 600, border: "none", cursor: "pointer" }}>Keep Swiping</button>
        </div>
      )}
      ...
      <div style={{ flex: 1, minHeight: 0, position: "relative", margin: "0 10px", marginBottom: 4 }}>
        <AnimatePresence mode="popLayout">
          <motion.div
            key={currentProfile.user_id}
            style={{ x, rotate, position: "absolute", inset: 0, borderRadius: 20, overflow: "hidden", touchAction: "none", cursor: "grab" }}
          ...
          >
            <img src={currentProfile.photos?.[photoIndex] || "/placeholder.jpg"} alt={currentProfile.display_name}
              style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", userSelect: "none", WebkitUserDrag: "none" } as any}
              draggable={false}
            ...
            {currentProfile.photos && currentProfile.photos.length > 1 && (
              <div style={{ position: "absolute", top: 8, left: 8, right: 8, display: "flex", gap: 3, zIndex: 5 }}>
            ...
            <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "linear-gradient(to top, rgba(0,0,0,0.9) 0%, rgba(0,0,0,0.5) 50%, transparent 100%)", padding: "40px 14px 12px", zIndex: 5 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                ...
                <Shield style={{ width: 14, height: 14, color: "#ec4899", marginLeft: 2 }} />
              </div>
              {currentProfile.city && (
                <p style={{ color: "rgba(255,255,255,0.6)", fontSize: 12, display: "flex", alignItems: "center", gap: 4, marginBottom: 3, margin: 0 }}>
                  <MapPin style={{ width: 11, height: 11 }} />{currentProfile.city}
                </p>
              )}
              {currentProfile.bio && (
                <p style={{ color: "rgba(255,255,255,0.7)", fontSize: 12, marginBottom: 3, margin: "3px 0", overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" } as any}>{currentProfile.bio}</p>
              )}
              {currentProfile.compatibility_score !== undefined && currentProfile.compatibility_score > 0 && (
                <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 3 }}>
                  ...
                  <span style={{ color: "#f59e0b", fontSize: 11, fontWeight: 600 }}>{currentProfile.compatibility_score}% match</span>
                </div>
              )}
              {currentProfile.interests && currentProfile.interests.length > 0 && (
                <div style={{ display: "flex", flexWrap: "wrap", gap: 3 }}>
                  {currentProfile.interests.slice(0, 4).map((interest: string) => (
                    <span key={interest} style={{ fontSize: 10, background: "rgba(255,255,255,0.15)", color: "rgba(255,255,255,0.8)", padding: "2px 7px", borderRadius: 999 }}>{interest}</span>
                  ))}
                </div>
              )}
            </div>
      ...
      <div style={{ flexShrink: 0, height: 68, display: "flex", alignItems: "center", justifyContent: "center", gap: 14 }}>
        {isPremium && canUndo && (
          <motion.button whileTap={{ scale: 0.85 }} onClick={() => handleUndo(isPremium)}
            style={{ width: 42, height: 42, borderRadius: "50%", background: "#1c1c1e", border: "1px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Undo2 style={{ width: 18, height: 18, color: "#f59e0b" }} />
          </motion.button>
        )}
        <motion.button whileTap={{ scale: 0.85 }} onClick={() => onSwipe("pass")} disabled={isProcessing}
          style={{ width: 52, height: 52, borderRadius: "50%", background: "#1c1c1e", border: "2px solid rgba(239,68,68,0.3)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <X style={{ width: 26, height: 26, color: "#ef4444" }} />
        </motion.button>
        <motion.button whileTap={{ scale: 0.85 }} onClick={() => onSwipe("superlike")} disabled={isProcessing}
          style={{ width: 42, height: 42, borderRadius: "50%", background: "#1c1c1e", border: "2px solid rgba(245,158,11,0.3)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <motion.button whileTap={{ scale: 0.85 }} onClick={() => onSwipe("like")} disabled={isProcessing}
          ...
          style={{ width: 52, height: 52, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", border: "none", boxShadow: "0 0 20px rgba(236,72,153,0.4)" }}>
