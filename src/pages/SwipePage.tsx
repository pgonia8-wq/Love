import { useState, useCallback, useMemo, useEffect } from "react";
  ...
  import { SpotlightBanner, CrushCounter, SocialProofBadges } from "@/components/FomoFeatures";
  ...
  export default function SwipePage({ userId, isPremium }: SwipePageProps) {
    ...
    if (!currentProfile) {
      return (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 2rem", textAlign: "center" }}>
          <Heart style={{ width: 40, height: 40, color: "#888", marginBottom: 16 }} />
          <h3 style={{ fontSize: 20, fontWeight: 600, marginBottom: 8 }}>No more profiles</h3>
    ...
    return (
      <>
        ...
        <AnimatePresence>
          {showLimitModal && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              style={{ position: "fixed", inset: 0, zIndex: 60, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.85)", padding: "0 2rem" }}>
              <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }}
                style={{ background: "var(--card, #1c1c1e)", borderRadius: 24, padding: 24, maxWidth: 320, width: "100%", textAlign: "center" }}>
                <Lock style={{ width: 40, height: 40, color: "#f59e0b", margin: "0 auto 12px" }} />
                <h3 style={{ fontSize: 20, fontWeight: 700, marginBottom: 8 }}>{t("dailyLimit.title")}</h3>
                <p style={{ color: "#888", fontSize: 14, marginBottom: 16 }}>{t("dailyLimit.desc", { limit: dailyLimit })}</p>
                {!isPremium && (
                  <button className="gradient-love" style={{ width: "100%", padding: "12px 0", borderRadius: 16, color: "#fff", fontWeight: 600, fontSize: 16, border: "none", marginBottom: 8 }}>
                    <Crown style={{ width: 16, height: 16, display: "inline", marginRight: 8 }} />{t("dailyLimit.upgrade")}
                  </button>
                )}
                <button onClick={() => setShowLimitModal(false)} style={{ color: "#888", fontSize: 14, background: "none", border: "none", padding: 8 }}>{t("dailyLimit.close")}</button>
              </motion.div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0, overflow: "hidden" }}>
          ...
          {/* Top bar: swipe counter + crush counter */}
          <div style={{ flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 12px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <Flame style={{ width: 14, height: 14, color: "#ec4899" }} />
              <span style={{ fontSize: 12, color: "#888" }}>{dailyCount}/{dailyLimit}</span>
            </div>
            <CrushCounter count={crushCount} onClick={() => {}} />
          </div>

          {/* Early adopter banner */}
          {showEarlyBanner && !isPremium && (
            <div style={{ flexShrink: 0, margin: "0 12px 6px", padding: "8px 12px", borderRadius: 12, background: "linear-gradient(135deg, rgba(236,72,153,0.15), rgba(168,85,247,0.15))", border: "1px solid rgba(236,72,153,0.3)" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <Crown style={{ width: 14, height: 14, color: "#f59e0b" }} />
                  <span style={{ fontSize: 12, fontWeight: 700, color: "#f59e0b" }}>{t("earlyAdopter.title")}</span>
                </div>
                <button onClick={() => setShowEarlyBanner(false)} style={{ color: "#888", fontSize: 16, background: "none", border: "none", lineHeight: 1 }}>×</button>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ fontSize: 9, color: "#f59e0b", fontWeight: 600, marginLeft: "auto" }}>{t("earlyAdopter.spotsLeft")}</span>
              </div>
          ...
          {/* Card area */}
          <div style={{ flex: 1, position: "relative", minHeight: 0, margin: "0 12px" }}>
            <AnimatePresence mode="popLayout">
              ...
              >
                ...
                <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "linear-gradient(to top, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.4) 60%, transparent 100%)", padding: "60px 14px 14px", zIndex: 5 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                    <span style={{ fontSize: 22, fontWeight: 700, color: "#fff" }}>{currentProfile.display_name}</span>
                    <span style={{ fontSize: 18, color: "rgba(255,255,255,0.8)" }}>{currentProfile.age}</span>
                    {(currentProfile as any).trust_score && <TrustBadge score={(currentProfile as any).trust_score} size="sm" />}
                  </div>
                  {currentProfile.city && (
                    <p style={{ color: "rgba(255,255,255,0.6)", fontSize: 13, display: "flex", alignItems: "center", gap: 4, marginBottom: 3 }}>
                      <MapPin style={{ width: 12, height: 12 }} />{currentProfile.city}
                    </p>
                  )}
                  {currentProfile.bio && (
                    <p style={{ color: "rgba(255,255,255,0.7)", fontSize: 13, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{currentProfile.bio}</p>
                  )}
                  <SocialProofBadges
                    responseTime={(currentProfile as any).avg_response_time_min}
                    ghostRate={(currentProfile as any).ghost_rate_pct}
                    matchRate={(currentProfile as any).match_rate_pct}
                  />
          ...
          {/* Action buttons */}
          <div style={{ flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", gap: 14, padding: "8px 16px", height: 68 }}>
            {isPremium && canUndo && (
              <motion.button whileTap={{ scale: 0.85 }} onClick={() => handleUndo(isPremium)}
                style={{ width: 42, height: 42, borderRadius: "50%", background: "var(--card,#1c1c1e)", border: "1px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <Undo2 style={{ width: 18, height: 18, color: "#f59e0b" }} />
              </motion.button>
            )}
            <motion.button whileTap={{ scale: 0.85 }} onClick={() => onSwipe("pass")} disabled={isProcessing}
              style={{ width: 52, height: 52, borderRadius: "50%", background: "var(--card,#1c1c1e)", border: "1px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            <motion.button whileTap={{ scale: 0.85 }} onClick={() => onSwipe("like")} disabled={isProcessing}
              ...
              <Heart style={{ width: 26, height: 26, color: "#fff", fill: "#fff" }} />
            </motion.button>
          </div>
        </div>
      </>
