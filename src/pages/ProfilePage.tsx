...
import { Shield, Crown, Zap, Edit2, LogOut, Camera, Heart, Star, TrendingUp, Share2, Check, Copy, MapPin, Navigation, Plane, Globe, Lock } from "lucide-react";
...
export default function ProfilePage({ user, profile, onUpdate, onLogout, onUpgrade }: ProfilePageProps) {
  ...
  const handleUpgrade = () => {
    ...
    else console.log("[Profile] No onUpgrade handler");
  };
    ...
    <div className="flex flex-col px-4 pt-4 pb-24">
      ...
      <div className={"rounded-2xl border p-4 mb-4 " + (travelActive ? "bg-love-purple/5 border-love-purple/20" : "bg-card border-border/30")}>
        ...
        {travelActive ? (
          ...
          <Button size="sm" onClick={() => isPremium ? setShowTravel(true) : handleUpgrade()} className={"w-full mt-2 rounded-xl h-8 text-xs " + (isPremium ? "gradient-love border-0" : "")}>
      ...
      <div className={"rounded-2xl border p-4 mb-4 " + (isPremium ? "bg-love-gold/5 border-love-gold/20" : "bg-muted/30 border-border/30")}>
        <div className="flex items-center justify-between">
          ...
          {!isPremium && <Button size="sm" onClick={handleUpgrade} className="gradient-love border-0 rounded-xl h-8 px-4 text-xs"><Crown className="w-3 h-3 mr-1" />Upgrade</Button>}
        </div>
      </div>

      <button onClick={handleUpgrade} className="flex items-center gap-3 p-4 bg-card rounded-2xl border border-border/30 mb-4 w-full text-left">
