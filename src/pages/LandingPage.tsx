...
export default function LandingPage({ onVerified }: LandingPageProps) {
  ...
  const handleVerify = async () => {
    ...
    console.log("[Landing] window.MiniKit:", !!(window as any).MiniKit);

    if (!MiniKit.isInstalled()) {
      console.error("[Landing] MiniKit NOT installed - not inside World App");
    ...
    try {
      ...
      console.log("[Landing] verify result status:", verifyResult?.finalPayload?.status);
      const finalPayload = verifyResult?.finalPayload;

      if (!finalPayload || finalPayload.status === "error") {
        console.error("[Landing] Verification failed:", JSON.stringify(finalPayload));
        setError("Verificacion cancelada o fallida");
        setIsPending(false);
        return;
      }
      ...
      console.log("[Landing] nullifier_hash:", successPayload.nullifier_hash);
      console.log("[Landing] merkle_root:", successPayload.merkle_root?.substring(0, 20) + "...");
      console.log("[Landing] verification_level:", successPayload.verification_level);
      console.log("[Landing] walletAddress:", walletAddress);
      ...
      if (!res.ok || !data.success) {
        console.error("[Landing] Backend rejected:", data.error, "code:", data.code, "details:", JSON.stringify(data.details));
        setError(data.error || "Error en la verificacion");
        setIsPending(false);
        return;
      }
