import { useState } from "react";
  ...
  export default function LandingPage({ onVerified }: LandingPageProps) {
    ...
    const handleVerify = async () => {
      ...
      try {
        // Check if user is Orb verified via MiniKit state (available at init)
        const isOrbVerified = MiniKit.user?.verificationStatus?.isOrbVerified;
        if (!isOrbVerified) {
          setError("Se requiere verificación Orb. Verifica tu identidad en World App.");
          setIsPending(false);
          return;
        }

        // Get nonce from backend for SIWE
        const nonceRes = await fetch("/api/nonce");
        const { nonce } = await nonceRes.json();

        // Authenticate with walletAuth (SIWE) - the recommended auth flow for mini apps
