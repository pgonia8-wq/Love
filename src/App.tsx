import { useState, useEffect, useCallback } from "react";
  ...
  function NavBar() {
    ...
    return (
      <div className="flex items-center justify-around py-2 px-2 border-t border-border/30 bg-card/50 backdrop-blur-xl flex-shrink-0">
        {tabs.map((tab) => {
    ...
    return (
      <div className="h-[100dvh] flex flex-col max-w-lg mx-auto overflow-hidden">
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
          <Switch>
            <Route path="/">
              <SwipePage userId={user.wallet_address} isPremium={user.is_premium} />
            </Route>
            <Route path="/matches">
              <MatchesPage userId={user.wallet_address} />
            </Route>
            <Route path="/chat/:matchId">
              <ChatPage userId={user.wallet_address} />
            </Route>
            <Route path="/events">
              <EventsPage userId={user.wallet_address} />
            </Route>
            <Route path="/wallet">
              <WalletPage user={user} userId={user.wallet_address} />
            </Route>
            <Route path="/profile">
              <ProfilePage
                user={user}
